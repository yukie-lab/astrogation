...
下)で交差する。この交差はこれまで定量化されていない(我々の知る限り。
検索範囲は第\ref{sec:intro}節)。$x^*$ の下では優勢エネルギーのフロンティアが
束縛し、上では厚壁実現可能性が束縛する。$x^*$ を超える薄殻カタログ値は
分布的理想化でのみ到達可能である。物理的内容は原論文自身の許容性窓の入れ子と
整合的であり、交差は操作レジームの切り替えを定量化したものである。
...
ファイル、図表生成器、数値マニフェスト、仮定台帳)は
\url{https://github.com/yukie-lab/astrogation} および
\url{https://doi.org/10.5281/zenodo.21853092} で公開し、プロジェクトリポジトリのタグ \texttt{v1.0-paper}(タグがリリース
コミットに解決される)に固定する。検証スイートは 1 分未満で走り、本論文の
すべての数値を再現する。
