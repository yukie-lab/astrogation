...
\author{Yukie Maeda\\ \small Independent Researcher, Tokyo\\
...
\label{tab:ladder}
\input{tables/table2_ladder.tex}
\end{table}
...
\label{tab:contour}
\input{tables/table3_contour.tex}
\end{table}
...
of the published pins), a crossover which has not, to our knowledge, been
quantified before (search scope in Sec.~\ref{sec:intro}): below $x^*$ the
dominant-energy frontier binds,
above it thick-wall realizability binds, and thin-shell catalog values
beyond $x^*$ are reachable only in the distributional idealization. The
...
\label{tab:bracket}
\input{tables/table4_bracket.tex}
\end{table}
...
manifest, and the assumptions ledger) is released at
\url{https://github.com/yukie-lab/astrogation} and
\url{https://doi.org/10.5281/zenodo.21853092}, pinned at tag
\texttt{v1.0-paper} of the project repository (the
tag resolves to the release commit). The verification suite runs in under
one minute and reproduces every number in this paper.
